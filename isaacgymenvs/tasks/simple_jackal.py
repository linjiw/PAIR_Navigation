...
class Jakcal(VecTask):
    ...
    def __init__(
        ...
        self.envs_id =  torch.arange(self.num_envs, device=self.device, dtype=torch.int32)
        # 
        self.init_done = True
        print_tensor_info(self.root_states, "Root States")
        print_tensor_info(self.dof_state, "DOF State")
    ...
    def _create_envs(self, num_envs):
        ...
        self.max_cylinders = 300  # Define this before initializing the cylinder pools
        self.cylinder_actor_idx = torch.zeros((self.num_envs, self.max_cylinders), device=self.device, dtype=torch.int32)

        # Pre-create cylinder pool for each environment
        # cylinder_pools = [[self.gym.create_actor(self.sim, cylinder_asset, gymapi.Transform(), "cylinder_pool", 0, False, False) for _ in range(max_cylinders)] for _ in range(self.num_envs)]
        # cylinder_pools = [self.gym.create_actor(env_handle, cylinder_asset, gymapi.Transform(), "cylinder_pool", i, -1, -1) for _ in range(max_cylinders)]
        ...
        self.all_gird_files = grid_files
        random_grid_files = np.random.permutation(grid_files * self.num_envs)[:self.num_envs]
        self.worlds = ["world_%s" %(f.split("_")[-1].split(".")[0]) for f in random_grid_files]
        for i in range(self.num_envs):
            env_handle = self.gym.create_env(self.sim, env_lower, env_upper, int(np.sqrt(self.num_envs)))
            # cylinder_pool = cylinder_pools[i]
            # cylinder_pool = [self.gym.create_actor(env_handle, cylinder_asset, gymapi.Transform(), f"cylinder_pool_{cylinder_id}", i, -1, -1) for cylinder_id in range(max_cylinders)]
            ...
            for x in range(1, grid.shape[0]-1):
                for y in range(1, grid.shape[1]-1):
                    # if grid[x][y]:
                    if cylinder_count < self.max_cylinders:
                        ...
                        self.cylinder_actor_idx[i, cylinder_count] = cylinder_count_total + i + cylinder_count
                        cylinder_count += 1
    ...
    def reset_cylinder_to_map(self, map_folder, if_random = True, if_order = -1):
        # for env_id/
        # for i in range(self.num_envs):
        env_id = self.envs_id.unsqueeze(0)
        # map_folder = self.cfg["env"]["terrain"]["mapFolder"]
        asset_root = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../assets')
        asset_file = self.cfg["env"]["urdfAsset"]["file"]
        asset_path = os.path.join(asset_root, asset_file)
        ...
        # load grid map
        cylinders_idx = self.cylinder_actor_idx[env_id]
        
        # print(f"cylinders_idx.shape {cylinders_idx.shape} ? {self.max_cylinders * self.num_envs}")
        # print(f"cylinder_idx {cylinders_idx}")
        # i = 0
        # for env_i in range(len(env_id)):
        # env_i = env_id[0]
        # cylinder_count = 0
        # print(f"env_i {env_i}")
        # curr_env_id = env_id[env_i]
        # all_gird_files
        if if_order != -1:
            grid_files = self.all_gird_files[if_order * self.num_envs: (if_order + 1) * self.num_envs]
        if if_random:
            random_env_id = np.random.randint(low = 0, high= len(self.all_gird_files) - 1, size= self.num_envs)
            random_grid_file = self.all_gird_files[random_env_id]
        print(f"grid_files {grid_files}")
        ...
        for i in range(self.num_envs):
            ...
            env_handle = self.envs[curr_env_id]
            if not if_random:
                grid_map = np.load(os.path.join(grid_root, grid_files[curr_env_id]))
            else:
                grid_map = np.load(os.path.join(grid_root, random_grid_file[i]))
            cylinder_pool = self.cylinder_actor_idx[curr_env_id]
            # for 
            for x in range(1, grid_map.shape[0]-1):
                for y in range(1, grid_map.shape[1]-1):
                    # i = x * grid_map.shape[0] + y
                    if grid_map[x][y] and cylinder_count < self.max_cylinders:
                        xx = x * 0.15; yy = y * 0.15 + 0.15 * 30
                        value = [xx, yy]
                        # print(f"setting cylindesr {cylinder_count} to {x}, {y}")

                        self.root_states[cylinder_pool[cylinder_count]][ :2] = torch.tensor(value , device = self.device)        
                        cylinder_count += 1

            # Handle any remaining cylinders in the pool
            for j in range(cylinder_count, self.max_cylinders):
                unused_cylinder_handle = cylinder_pool[j]
                self.root_states[unused_cylinder_handle][ :2] = torch.tensor([0, 0], device = self.device)
            cylinders_idx = self.cylinder_actor_idx[self.envs_id]
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            # gymtorch.unwrap_tensor(self.envs_id), len(self.envs_id)
        )
