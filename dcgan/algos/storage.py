...
class RolloutStorage(object):
	def __init__(self, 
		...
		print(f"if isinstance(observation_space, dict): {isinstance(observation_space, dict)}")
		print(f"observation_space.shape: {observation_space.shape}")
		print(f"items: {observation_space.spaces.items()}")
		...
		if isinstance(observation_space, gym.spaces.Dict):
			...
			if self.use_proper_time_limits:
				self.truncated_obs = {k: torch.zeros(num_steps + 1, num_processes, *obs_space.shape) 
									  for k, obs_space in observation_space.spaces.items()}
		else:
			self.is_dict_obs = False
			self.obs = torch.zeros(num_steps + 1, num_processes, *observation_space.shape)
		...
		# print(f"init self.rewards.shape: {self.rewards.shape}")
	...
	def copy_obs_to_index(self, obs, index):
		if self.is_dict_obs:
			# print(f"obs['random_z']: {obs['random_z'].shape}")
			# print(f"index: {index}")
			# print(f"self.obs['random_z'].shape: {self.obs['random_z'].shape}")
			# print(f"self.obs['random_z'][index].shape: {self.obs['random_z'][index].shape}")
	...
	def insert(self, obs, recurrent_hidden_states, actions, action_log_probs, action_log_dist,
		...
		if self.is_lstm:
			...
			print(f"self.step + 1: {self.step + 1}")
			print(f"self.recurrent_hidden_states.shape: {self.recurrent_hidden_states.shape}")
		...
		# print(f"self.action_log_probs {self.action_log_probs.shape}")
	...
	def after_update(self):
		...
		# print(f"after update self.actions.shape: {self.actions.shape}")

		# print(f"after update self.actions.shape: {self.actions.cpu().numpy()}")
				# Calculate statistics
		# std = torch.std(self.actions).item()
		# mean = torch.mean(self.actions).item()
		# min_val = torch.min(self.actions).item()
		# max_val = torch.max(self.actions).item()
		...
		# plot_and_save(self.actions, std, 'Standard Deviation')
	...
	def replace_obs(self, obs):
		...
		# print(f"old obs shape: {self.obs['obs'].shape}")
		# print(f"target obs: {obs}")
		if self.is_dict_obs:
			# [self.obs[k][0][1].copy_(obs[k]) for k in self.obs.keys()]
			self.obs['obs'][-1][0][1] = obs
		else:
			self.obs[0].copy_(obs)
		# print(f"new obs: {self.obs}")
